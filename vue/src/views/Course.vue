<template>
  <div style="padding: 10px">
    ...
    <div style="margin: 10px 0">
      <el-button type="primary" @click="add" v-if="user.isAdmin === 1">新增</el-button>
      <el-popconfirm
          v-if="user.isAdmin === 1"
          title="确定删除吗？"
          @confirm="deleteBatch"
    ...
    >
      ...
      <el-table-column label="操作" width="240">
        <template #default="scope">
          ...
          <el-button size="mini" @click="handleEdit(scope.row)" v-if="user.isAdmin === 1">编辑</el-button>
          <el-popconfirm title="确定删除吗？" @confirm="handleDelete(scope.row.id)" v-if="user.isAdmin === 1">
            <template #reference>
...
export default {
  ...
  methods: {
    buy(courseId) {
      request.get("/order/buy/" + courseId).then(res => {
        // 请求成功跳转沙箱支付的页面
    ...
    deleteBatch() {
      ...
      request.post("/course/deleteBatch", this.ids).then(res => {
        if (res.code === '0') {
